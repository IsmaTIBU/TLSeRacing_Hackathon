...
from fsd_path_planning.calculate_path.dumb_calculate_path import DumbCalculatePath
...
def get_cone_fitting_config(
    ...
    return dict(smoothing=0.0, predict_every=0.1, max_deg=1)
...
def create_default_pathing(mission: MissionTypes) -> DumbCalculatePath:
    """
    ...
    possible_path_calculation_classes: Dict[MissionTypes, Type[CalculatePath]] = {
        MissionTypes.skidpad: DumbCalculatePath,
    }

    path_calculation_class = possible_path_calculation_classes.get(
        mission, DumbCalculatePath
    )
...
def get_default_matching_kwargs(mission: MissionTypes) -> KwargsType:
    ...
    return dict(
        min_track_width=1.5,
        max_search_range=5,
        max_search_angle=np.deg2rad(60),
